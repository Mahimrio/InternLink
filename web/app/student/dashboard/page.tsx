...
export default function StudentDashboardPage() {
  ...
  const fullName = profile ? `${profile.firstName} ${profile.lastName}`.trim() : user?.name || "Student";
  ...
  return (
    <PageContainer className="py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="fixed top-20 right-1/4 -z-10 h-96 w-96 rounded-full bg-teal-400/10 blur-[130px] pointer-events-none" />
      <div className="fixed bottom-10 left-10 -z-10 h-96 w-96 rounded-full bg-amber-400/10 blur-[140px] pointer-events-none" />

      {/* Hero Welcome Card - Ultra-Clean Glassmorphism */}
      <div className="relative overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/85 dark:bg-slate-900/85 backdrop-blur-xl shadow-lg shadow-teal-950/5 p-6 sm:p-8">
        {/* Top Gradient Shimmer Line */}
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-teal-600 via-teal-400 to-amber-400" />

        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="flex items-start gap-4 sm:gap-5">
            {/* Avatar Badge */}
            <div className="size-14 sm:size-16 rounded-2xl bg-gradient-to-br from-teal-600 to-teal-800 text-white font-heading font-bold text-xl sm:text-2xl flex items-center justify-center shadow-md shadow-teal-700/20 shrink-0 border-2 border-white/80 dark:border-slate-800">
              {initials}
            </div>

            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200/60 shadow-2xs">
                  <span className="size-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  Verified Student
                </span>
                {profile?.institutionalId && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    <GraduationCap className="size-3.5 text-slate-500" />
                    ID: {profile.institutionalId}
                  </span>
                )}
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs text-slate-600 dark:text-slate-400 bg-slate-100/60 dark:bg-slate-800/60">
                  <ShieldCheck className="size-3.5 text-teal-600" />
                  CSE 3200 & 3224
                </span>
              </div>

              <h1 className="font-heading text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white text-balance">
                Welcome back,{" "}
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 max-w-2xl leading-relaxed text-pretty">
                {profile?.department || "Department of Computer Science & Engineering"} • Ahsanullah University of Science & Technology. Build ATS-optimized resumes with QuestPDF, explore verified internships, and track applications.
              </p>
              ...
              <div className="pt-2 max-w-md space-y-1.5">
                ...
                <div className="h-2 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-200/60 dark:border-slate-700/60">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-teal-500 to-amber-400 transition-all duration-1000 ease-out shadow-xs"
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-2.5 shrink-0 pt-2 lg:pt-0">
            <Link href="/student/resumes/builder">
              <Button className="w-full bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white font-semibold shadow-md shadow-teal-700/20 btn-gradient-animate transition-transform active:scale-[0.98]">
            ...
            <Link href="/student/profile">
              <Button
                variant="outline"
                className="w-full border-slate-200 dark:border-slate-700 bg-white/70 dark:bg-slate-800/70 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-medium backdrop-blur-sm"
              >
      ...
      {/* Metrics Row - 4 Glassmorphic Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Metric 1: Active Applications */}
        <div className="group relative rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-md hover:border-teal-400/50 hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="mt-3">
            <div className="font-heading text-2xl font-bold text-slate-900 dark:text-white tabular-nums">0</div>
        ...
        <div className="group relative rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-md hover:border-teal-400/50 hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="mt-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5">
              <span className="size-1.5 rounded-full bg-teal-500 inline-block" />
              QuestPDF compiled cloud drafts
            </p>
          </div>
        </div>

        {/* Metric 3: Skills Matrix */}
        <div className="group relative rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-md hover:border-teal-400/50 hover:-translate-y-1 transition-all duration-300">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              Skills Matrix
          ...
          <div className="mt-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1.5">
              <span className="size-1.5 rounded-full bg-teal-500 inline-block" />
              Relational skill records
            </p>
          </div>
        </div>

        {/* Metric 4: Academic CGPA */}
        <div className="group relative rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-md hover:border-amber-400/50 hover:-translate-y-1 transition-all duration-300">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              Current CGPA
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500">Student Tools</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Link
                ...
                className="group rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-lg hover:border-teal-400 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between">
                ...
                <div className="pt-4 flex items-center gap-1 text-xs font-semibold text-teal-700 dark:text-teal-400">
                  Open wizard <ArrowRight className="size-3.5 group-hover:translate-x-1 transition-transform" />
                </div>
              </Link>
              ...
              <Link
                ...
                className="group rounded-xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md p-5 shadow-2xs hover:shadow-lg hover:border-amber-400 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between">
          ...
          <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md shadow-2xs p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
            ...
            {resumes.length === 0 ? (
              <div className="py-10 text-center rounded-xl bg-slate-50/50 dark:bg-slate-800/30 border border-dashed border-slate-200 dark:border-slate-800">
                ...
                <Link href="/student/resumes/builder" className="inline-block mt-4">
                  <Button size="sm" className="bg-teal-700 text-white text-xs shadow-xs">
                    Create Resume
                  </Button>
                </Link>
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {r.downloadUrl ? (
                        <a href={r.downloadUrl} target="_blank" rel="noopener noreferrer">
                          <Button size="sm" className="h-8 text-xs bg-teal-700 text-white hover:bg-teal-800 shadow-2xs">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md shadow-2xs p-6 space-y-4">
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-200/60">
          ...
          <div className="rounded-2xl border border-slate-200/70 dark:border-slate-800/70 bg-white/75 dark:bg-slate-900/75 backdrop-blur-md shadow-2xs p-5 space-y-2">
